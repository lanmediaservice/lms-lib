...
class Lms_MetaParser {
    ...
    public static function setInstances($instances)
    {
        if (is_array($instances)) { 
            ...
            throw new Exception("Can't set Instances. The type of param must be an array."); 
        } 
    }
        ...
        if ($fileSize > pow(2, 31)) {
            ...
            for ($i = 0; $i < count($demuxers); $i++) {
                ...
                if (in_array($extension, self::$_config['fileExtensions'][$demuxer])) {
                    ...
                    throw new Exception("The extension: $extension of file you are parsing is not assign to demuxer: $demuxer");
                }
            }
        } else {
